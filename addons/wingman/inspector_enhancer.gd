...
func on_inspector_edited_object_changed():
	...
	if inspector.get_edited_object() is AnimationNodeStateMachineTransition:
		var sections: Array[Node] = inspector.find_children(
			"*", "EditorInspectorSection", true, false
		)
		if sections.size() > 0:
			sections[0].unfold()
		if sections.size() > 1:
...
func get_inspector_property_filter_bar() -> HBoxContainer:
	for control in inspector.get_parent().get_children() as Array[Control]:
		if (
			control.get_child_count() > 1
			&& control.get_child(1) is MenuButton
			&& control.get_child(1).icon == base_control.get_theme_icon("Tools", "EditorIcons")
		):
			return control
	return null
