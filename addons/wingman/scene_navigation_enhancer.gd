...
func perform():
	...
	scene_selector.get_popup().id_pressed.connect(
		func(idx: int):
	...
	scene_selector.add_theme_font_override(
		"font", editor_selector.get_child(0).get_theme_font("font")
	)
	scene_selector.add_theme_font_size_override(
		"font_size", editor_selector.get_child(0).get_theme_font_size("font_size")
	)
...
func generate_scenes_info(scene_file_tree: Dictionary) -> Array:
	...
	if dir_title != "res://":
		...
		dir_indent += 1
	scenes_info.push_back(
		{
			"type": "dir",
			"path": scene_file_tree["path"],
			"title": dir_title,
			"indent": dir_indent,
			"class": "Folder"
		}
	...
	if scene_file_tree.has("files"):
		for file_path in scene_file_tree["files"]:
			...
			if !scenes_instantiate.has(file_path):
				...
				scenes_instantiate[file_path] = {
					"title": scene_name, "class": scene_class, "editor_type": editor_type
				}
			var info = {"type": "file", "path": file_path, "indent": file_indent}.merged(
				scenes_instantiate[file_path]
			)
