...
func change_setting(
	value: Variant, setting: String, update_ui: bool = false, update_setting: bool = true
):
	...
	match setting:
		"image":
			...
			if is_prev_ready:
				tool.preview.texture = img
			if update_setting:
				bg.texture = img
		"stretch":
			if update_setting:
				bg.stretch_mode = value
			if is_prev_ready:
				tool.preview.stretch_mode = value
				if update_ui:
					tool.get_node("VBoxContainer/HBoxContainer/VBoxContainer/stretch mode").select(
						value
					)
		"filter":
			if update_setting:
				bg.texture_filter = value
			if is_prev_ready:
				tool.preview.texture_filter = value
				if update_ui:
					tool.get_node("VBoxContainer/HBoxContainer/VBoxContainer/filter mode").select(
						value
					)
		"ui_color":
			if is_prev_ready:
				if update_ui:
					...
					value = (
						tool.get_node("VBoxContainer/HBoxContainer/VBoxContainer2/ui_color").color
					)
			if not update_ui && update_setting:
				if value == Color(settings["ui_color"]):
					return
		...
		"edit_transparency":
			...
			if not update_ui && update_setting:
				if value == settings["edit_transparency"]:
					return
